...
**Left off at**: Mac `main` is pushed at `0f6f46b`; the companion is pushed at `07c2419`, with `0.1.7` still the latest release tag at `a30439f`. The Apple Note `💻 Tech/🌉 Skylight Bridge` was refreshed to match the live implementation. The remaining end-to-end check is on physical signed devices.
